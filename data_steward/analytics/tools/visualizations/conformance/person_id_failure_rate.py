...
# plt.xlabel(xlabel="\nTable Type", size=16)
# plt.title("Person ID Failure Rates for {}".format(date), size = 18)
# sns.despine(trim=True, left=True)

# # Now let's look at the metrics for particular sites with respect to person ID failure rate; this will allow us to send them the same information
...
plt.title("Person ID Failure Rates: {}".format(name_of_interest), size=15, y = 1.1)
...
plt.title("{} Person ID Failure Rates Over Time".format(name_of_interest))
plt.ylabel("Person ID Failure Rate (%)")
plt.xlabel("")
...
img_name = name_of_interest + "_person_id_failure_rate_line_graph.jpg"
