...
SELECT
DISTINCT
mm.src_hpo_id, COUNT(m.unit_concept_id) as number_total_units
FROM
...
SELECT
DISTINCT
mm.src_hpo_id, COUNT(m.unit_concept_id) as number_valid_units
FROM
`{DATASET}.unioned_ehr_measurement` m
JOIN
`{DATASET}._mapping_measurement` mm
...
SELECT
DISTINCT
mm.src_hpo_id, COUNT(m.unit_concept_id) as number_total_units_selected_measurements
FROM
`{DATASET}.unioned_ehr_measurement` m
JOIN
`{DATASET}._mapping_measurement` mm
...
final_all_units_df['total_proportion_of_units_that_are_selected'] = round(final_all_units_df['number_total_units_selected_measurements'] / final_all_units_df['number_total_units'] * 100, 2)
