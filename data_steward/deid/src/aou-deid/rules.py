...
class Rules(object):
    ...
    def __init__(self):
>>>>>>> 81d32a38db3c93405897571ab8f97da96ffc6d6d
        self.cache = {}
        self.store_syntax = {
...
>>>>>>> 81d32a38db3c93405897571ab8f97da96ffc6d6d
                                ...
                                try:
                                    regex += args['key_row'] + " IN ('"+ "','".join(rule['on']) + "')"
                                except TypeError:
                                    vals = [str(on_val) for on_val in rule['on']]
                                    regex += args['key_row'] + " IN (" + ", ".join(vals) + ")"
...
=======
                    ...
                    if isinstance(_into, int):
                        _into = str(_into)
                    elif not _into.isnumeric():
                        _into = "'" + _into + "'"
...
=======
                    ...
                    filtter = args.get('filter', name)
                    qualifier = rule.get('qualifier', '')

                    try:
                        values = "('" + "','".join(rule['values']) +"')"
                    except TypeError:
                        ints_to_strings = [str(val) for val in rule['values']]
                        values = "(" + ', '.join(ints_to_strings) + ")"

                    statement = " ".join([key_field, qualifier, values])
                    _into = rule['into'] if 'into' not in args else args['into']
                    regex = " ".join([filtter, qualifier, values])
                    if isinstance(_into, int):
                        _into = str(_into)
                    elif not _into.isnumeric():
                        _into = "'" + _into + "'"
...
>>>>>>> 81d32a38db3c93405897571ab8f97da96ffc6d6d
    ...
    def shift(self, **args):
        ...
        #   - simply specified are physical date/datetime columns (nothing special here)
